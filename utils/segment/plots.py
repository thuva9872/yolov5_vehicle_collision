...
def plot_masks(img, masks, colors, alpha=0.5):
    ...
    for i in range(masks.shape[0]):
        bounder = find_boundaries(masks[i,:,:].cpu().numpy(), mode='thick').astype(np.uint8)
        bounder[0:5*(bounder.shape[0] // 20), :] = 0
        # bounder[:, 0:5*(bounder.shape[0] // 20)] = 0
        a180 = np.rot90(m=bounder, k=2)
        point_1 = a180.shape[0] * a180.shape[1] - np.ndarray.argmax(a180) - 1
        r1, c1 = point_1 // bounder.shape[1], point_1 % bounder.shape[1]
        a90 = np.rot90(m=bounder, k=1)
        point_2 = (np.ndarray.argmax(a90) % a90.shape[1]) * a90.shape[0] + a90.shape[0] - (
                    np.ndarray.argmax(a90) // a90.shape[1]) - 1
        r2, c2 = point_2 // bounder.shape[1], point_2 % bounder.shape[1]
    ...
    # # SLIC result
    # slic = segmentation.slic(img.numpy(), n_segments=200, start_label=1)
    # cv2.imshow('window_name', img.numpy())
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    # # maskSLIC result
    # m_slic = segmentation.slic(img.numpy(), n_segments=100, mask=masks[0, :, :, 0].numpy(), start_label=1)
